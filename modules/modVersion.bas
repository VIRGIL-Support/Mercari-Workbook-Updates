...
Private Sub PerformUpdate(ByVal newVersion As String)
    ...
    MsgBox "All files downloaded successfully!" & vbCrLf & vbCrLf & _
           "The update will now be installed." & vbCrLf & _
...
Private Sub InstallUpdate(ByVal tempFolder As String, ByVal newVersion As String)
    ...
    ' Update modules by replacing code in place (no remove/import)
    For Each file In folder.Files
        If LCase$(fso.GetExtensionName(file.Name)) = "bas" Then
            ...
            ' Read the new module code from file
            fileNum = FreeFile
            Open filePath For Input As #fileNum
            fileContent = Input$(LOF(fileNum), fileNum)
            Close #fileNum
            
            ' Strip the Attribute VB_Name line from the file content
            codeStart = InStr(fileContent, vbCrLf)
            If codeStart > 0 Then
                fileContent = Mid$(fileContent, codeStart + 2)
            End If
            ...
            ' Replace all code in the module
...
NextFile:
    ...
    ' Save workbook with updated modules
    ThisWorkbook.Save
    
    MsgBox "Update complete! All modules have been updated to version " & newVersion & "." & vbCrLf & vbCrLf & _
           "The workbook has been saved.", vbInformation, "Update Successful"
