...
Private Sub DownloadUpdate(ByVal newVersion As String)
    ...
    ' Create a VBScript that waits then opens the new workbook
    Dim vbsPath As String
    Dim vbsFile As Integer
    vbsPath = Environ$("TEMP") & "\MercariOpenUpdate.vbs"
    vbsFile = FreeFile
    Open vbsPath For Output As #vbsFile
    Print #vbsFile, "WScript.Sleep 3000"
    Print #vbsFile, "On Error Resume Next"
    Print #vbsFile, "Set xlApp = GetObject(,""Excel.Application"")"
    Print #vbsFile, "If Err.Number <> 0 Then"
    Print #vbsFile, "  Err.Clear"
    Print #vbsFile, "  Set xlApp = CreateObject(""Excel.Application"")"
    Print #vbsFile, "  xlApp.Visible = True"
    Print #vbsFile, "End If"
    Print #vbsFile, "xlApp.Workbooks.Open """ & downloadPath & """"
    Print #vbsFile, "Set fso = CreateObject(""Scripting.FileSystemObject"")"
    Print #vbsFile, "fso.DeleteFile """ & vbsPath & """, True"
    Close #vbsFile
    
    ' Launch the script and close this workbook
